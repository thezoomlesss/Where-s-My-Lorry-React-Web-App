...
connection.connect(function (error) {
    // callback
    if (!!error) {
        console.log('Error when connecting from getVehicleOrigin.js');

        console.log(error);
    } else {
        console.log('Db connected from getVehicleOrigin.js!');
    }
});
...
router.get('/', function (req, res, next) {
    ...
    if (company_id) {
        connection.query(`
            SELECT lp.country_origin, count(lp.country_origin) as number FROM License_Plate lp
            JOIN Vehicle v Using(plateID)
            JOIN Company Using(companyID)
            WHERE companyID = ${1}
            Group By country_origin
            ORDER BY count(lp.country_origin) DESC;`, {company_id}, function (error, results, fields) {
